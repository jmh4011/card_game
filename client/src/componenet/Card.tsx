...
import baseSprites from '../assets/image/base_sprites.png';
...
const Name = styled(IconLink)`
  ...
  font-size: ${({ scale, children }) => {
    ...
    return length < 15 ? `${60 * scale}px` : `${30 * scale}px`; // 길이에 따라 폰트 크기 조절
  }};
`;
...
const Text = styled(IconLink)`
  ...
  font-size: ${({ scale, children }) => {
    const length = (children as string).length;
    return length < 50 ? `${50 * scale}px` : `${30 * scale}px`; // 길이에 따라 폰트 크기 조절
  }};
