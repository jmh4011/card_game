...
class GameServices:
    ...
    async def check_user(db: AsyncSession, user_id:int):
        ...
        if user_stat is None:
            return False
        user_deck = await UserServices.get_deck_selection()
    ...
    async def connect_user(db: AsyncSession, websocket: WebSocket, user_id: int):
        ...
        deck_id = await UserDeckSelectionCrud.get(db=db, user_id=user_id, mod_id=user_info.current_mod_id)
        player = Player(user_id=user_id, websocket=websocket, deck_id=deck_id)
