...
class DeckServices:
    ...
    async def get(db: AsyncSession, player_id: int) -> Deck:
        decks = await handle_transaction(db=db, func=DeckCrud.get_all, player_id=player_id)
        for deck in decks:
    ...
    async def update(db: AsyncSession, player_id:int, deck_id: int, deck: RouterDeckUpdate) -> RouterDeckUpdateReturn:
        ...
        if db_deck is None or db_deck.player_id != player_id:
            return None
        result_deck = await handle_transaction(db=db, func=DeckCrud.update, should_refresh=True,deck_id=deck_id, deck=DeckUpdate(player_id=player_id,deck_name=deck.deck_name, image=deck.image))
